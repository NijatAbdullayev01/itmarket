/**
 * One-shot import: Aruba / Instant On products from aruba.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the HPE/Aruba part number. Each P/N is a separate product.
...
} from '../src/catalog/aruba-product-name';
import {
  buildArubaProductDescription,
  resolveArubaProductSeo,
} from '../src/catalog/aruba-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'aruba.xlsx');
...
const SUBCATEGORY_SLUG_BY_LABEL: Record<string, string> = {
  ...
  'idare olunmayan kommutator': 'kommutator',
  'smart cloud kommutator': 'kommutator',
  'enterprise kommutator': 'kommutator',
...
const BRAND_LOGO_URLS = [
  'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4a/Aruba_Networks_logo.svg/640px-Aruba_Networks_logo.svg.png',
  'https://upload.wikimedia.org/wikipedia/commons/4/4a/Aruba_Networks_logo.svg',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeArubaSku(cellText(raw[4]));
...
function assertSku(model: string): string {
  const sku = normalizeArubaSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `aruba-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `aruba-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `aruba-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('Aruba brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'Aruba' };
  }
  if (existing.status !== CatalogStatus.ACTIVE || existing.name !== 'Aruba') {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'Aruba', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'Aruba' };
}
...
async function importArubaProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveArubaCatalogName(sku, row.title, {
        subcategorySlug,
        specs,
      });
      const seo = resolveArubaProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`aruba ${sku}`);
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildArubaProductDescription(seo.pageIntro, specs),
              warrantyMonths,
              status: CatalogStatus.ACTIVE,
...
void importArubaProducts().catch((error: unknown) => {
  process.stderr.write(
