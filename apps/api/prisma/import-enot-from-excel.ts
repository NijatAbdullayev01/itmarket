/**
 * One-shot import: ENOT UPS batteries from ENOT_Batareyaları.xlsx
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'ENOT_Batareyaları.xlsx');

const PARENT_SLUG = 'ups';
const SUBCATEGORY_SLUG = 'ups-batareyalari';
const SUBCATEGORY_NAME = 'UPS batareyaları';

const SUBCATEGORY_SLUG_BY_LABEL: Record<string, string> = {
  'ups batareyalari': SUBCATEGORY_SLUG,
  'ups batareyaları': SUBCATEGORY_SLUG,
};

/** pcshop.ge is Cloudflare-blocked; ITbox product photos are used as fallback. */
const FALLBACK_IMAGE_BY_SKU: Record<string, string> = {
  'NP12-12': 'https://img5.itbox.ua/700x700/prod_img/5/U0092495_1720653399.jpg',
  'NP5.0-12': 'https://img8.itbox.ua/700x700/prod_img/8/U0092498_1719857287.jpg',
  'NP7.0-12': 'https://img0.itbox.ua/700x700/prod_img/0/U0092500_1719857289.jpg',
  'NP7.5-12': 'https://img1.itbox.ua/700x700/prod_img/1/U0092501_1763160394.jpg',
};
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `enot-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `enot-out-${randomUUID()}.jpg`);
...
function imageCandidateUrls(imageUrl: string, sku: string): string[] {
  const urls = [imageUrl];
  if (imageUrl.startsWith('https://pcshop.ge/')) {
    urls.push(`https://web.archive.org/web/2020/${imageUrl}`);
    urls.push(`https://web.archive.org/web/2024/${imageUrl}`);
  }
  const fallback = FALLBACK_IMAGE_BY_SKU[sku];
...
async function fetchImageBody(imageUrl: string): Promise<Buffer | null> {
  const response = await fetch(imageUrl, {
    ...
    headers: {
      ...
      Referer: imageUrl.includes('itbox.ua')
        ? 'https://www.itbox.ua/'
        : imageUrl.includes('web.archive.org')
...
async function downloadImage(
  ...
  for (const candidate of imageCandidateUrls(imageUrl, sku)) {
    body = await fetchImageBody(candidate);
    if (body !== null) {
      break;
    }
  }
...
function assertSku(model: string): string {
  const sku = normalizeEnotSku(model);
...
async function ensureBrand(
  ...
  if (existing !== null) {
    if (existing.status !== CatalogStatus.ACTIVE || existing.name !== 'ENOT') {
      await prisma.brand.update({
        where: { id: existing.id },
        data: { name: 'ENOT', status: CatalogStatus.ACTIVE },
      });
    }
    return { id: existing.id, name: 'ENOT' };
  }
...
async function ensureUpsBatteryCategory(
  prisma: PrismaClient,
): Promise<string> {
...
async function importEnotProducts(): Promise<void> {
  ...
  try {
    ...
    const categoryId = await ensureUpsBatteryCategory(prisma);

    for (const row of rows) {
      if (row.brand.toUpperCase() !== 'ENOT') {
      ...
      const productName = resolveEnotCatalogName(sku, row.title);
      ...
      const productSlugBase = slugifyCatalogLabel(`enot ${sku}`);
      ...
      const media = await downloadImage(row.imageUrl, sku);
...
void importEnotProducts().catch((error: unknown) => {
  process.stderr.write(
