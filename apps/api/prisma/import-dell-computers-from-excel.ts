/**
 * One-shot import: Dell client products from dellkomp.xlsx
 ...
 * Photos prefer the Excel URL (Icecat / Dell SNP), then SNP by P/N, then the
 * embedded Excel packshot for that row.
 */
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'dellkomp.xlsx');
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `dell-computers-catalog-${randomUUID()}`);
...
async function resolveProductImage(
  ...
  const candidates = [imageUrl, snpPartImageUrl(sku)].filter(
    (url, index, list) => url !== '' && list.indexOf(url) === index,
  );
  for (const candidate of candidates) {
    const remote = await fetchRemoteImage(candidate);
    if (remote !== null) {
...
async function importDellComputerProducts(): Promise<void> {
...
void importDellComputerProducts().catch((error: unknown) => {
  process.stderr.write(
