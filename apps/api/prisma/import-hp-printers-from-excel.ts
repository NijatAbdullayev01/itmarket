/**
 * One-shot import: HP printers from hpprinter.xlsx
 * Qiymət AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the HP part number (SKU / P/N). Photos prefer the official URL,
 * then the embedded Excel packshot for that row.
 */
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'hpprinter.xlsx');
...
const SUB_SLUG_BY_PARENT_AND_LABEL: Record<string, Record<string, string>> = {
  printerler: {
    ...
    'lazer mfp': 'lazer-mfp',
    'rengli lazer printer': 'rengli-lazer-printer',
...
const SUBCATEGORIES_TO_ENSURE: Array<{
  ...
  { parentSlug: 'printerler', slug: 'inkjet-mfp', name: 'Inkjet MFP' },
  ...
  { parentSlug: 'printerler', slug: 'lazer-mfp', name: 'Lazer MFP' },
  {
    ...
    slug: 'rengli-lazer-printer',
    name: 'Rəngli lazer printer',
  },
  {
    parentSlug: 'printerler',
  ...
  { parentSlug: 'printerler', slug: 'skaner', name: 'Skaner' },
...
function parseSpecs(features: string): Array<{ label: string; value: string }> {
  ...
  for (const rawLine of features.split(/\r?\n/)) {
    ...
    const labelKey = label.toLocaleLowerCase('az');
    if (SKIP_SPEC_LABELS.has(labelKey)) {
      continue;
    }
    if (labelKey === 'kartric' && value.length > 400) {
      continue;
    }
    if (labelKey === 'uyğunluq' && /work with:\s*$/i.test(value)) {
      continue;
    }
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `hp-printer-catalog-${randomUUID()}`);
...
async function fetchRemoteImage(imageUrl: string): Promise<Buffer | null> {
  ...
  for (const candidate of imageUrlFallbacks(imageUrl)) {
    for (const headers of headerSets) {
      const raw = await fetchImageBody(candidate, headers);
      if (raw !== null) {
        return raw;
      }
    }
...
async function importHpPrinters(): Promise<void> {
...
void importHpPrinters().catch((error: unknown) => {
  process.stderr.write(
