/**
 * One-shot import: Xerox products from xerox.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Qiymət Sorğu əsasında → variant.price/cost 0 (storefront: Sorğu əsasında).
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the Xerox (or G&G) part number.
 * G&G-106R02773 is a compatible cartridge — imported under the G&G brand.
 * Photos prefer Icecat URL, then the embedded packshot for that row.
 */
...
import {
  buildXeroxVariantAttributes,
  buildXeroxVariantName,
  normalizeXeroxSku,
  resolveXeroxCatalogName,
  resolveXeroxImportBrand,
  type XeroxImportBrand,
} from '../src/catalog/xerox-product-name';
import {
  buildXeroxProductDescription,
  resolveXeroxProductSeo,
} from '../src/catalog/xerox-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'xerox.xlsx');
...
const SUB_SLUG_BY_PARENT_AND_LABEL: Record<string, Record<string, string>> = {
  printerler: {
    'rengli lazer mfp': 'rengli-lazer-mfp',
    'lazer mfp': 'lazer-mfp',
...
const SUBCATEGORIES_TO_ENSURE: Array<{
...
}> = [
  {
    ...
    slug: 'rengli-lazer-mfp',
    name: 'Rəngli lazer MFP',
  },
  { parentSlug: 'printerler', slug: 'lazer-mfp', name: 'Lazer MFP' },
  { parentSlug: 'printerler', slug: 'lazer-printer', name: 'Lazer printer' },
  { parentSlug: 'printerler', slug: 'kartric', name: 'Kartric' },
];
...
const BRAND_LOGO_URLS: Record<string, readonly string[]> = {
  xerox: [
    'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9f/Xerox_logo.svg/640px-Xerox_logo.svg.png',
    'https://upload.wikimedia.org/wikipedia/commons/9/9f/Xerox_logo.svg',
  ],
};
...
function isUnlistedPrice(value: string): boolean {
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeXeroxSku(partNumber);
...
function assertSku(model: string): string {
  const sku = normalizeXeroxSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `xerox-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `xerox-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `xerox-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  prisma: PrismaClient,
  brand: XeroxImportBrand & { id: string; logoObjectKey: string | null },
): Promise<void> {
  if (brand.logoObjectKey !== null && brand.logoObjectKey !== '') {
    return;
  }
  const urls = BRAND_LOGO_URLS[brand.slug] ?? [];
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    await attachBrandLogoIfMissing(prisma, {
      ...meta,
      id: created.id,
  ...
  if (existing.status !== CatalogStatus.ACTIVE || existing.name !== meta.name) {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: meta.name, status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, {
    ...meta,
  ...
  return { id: existing.id, name: meta.name };
}
...
async function importXeroxProducts(): Promise<void> {
  ...
  try {
    ...
    for (const meta of [
      resolveXeroxImportBrand('C325V_DNI'),
      resolveXeroxImportBrand('GG-106R02773'),
    ]) {
      if (!brandBySlug.has(meta.slug)) {
        brandBySlug.set(meta.slug, await ensureBrand(prisma, meta));
      }
    }
    ...
    for (const row of rows) {
      ...
      const brandMeta = resolveXeroxImportBrand(sku);
      let brand = brandBySlug.get(brandMeta.slug);
      if (brand === undefined) {
        brand = await ensureBrand(prisma, brandMeta);
        brandBySlug.set(brandMeta.slug, brand);
      }
      ...
      const productName = resolveXeroxCatalogName(sku, row.title);
      const seo = resolveXeroxProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const attributes = buildXeroxVariantAttributes(specs, sku);
      const variantName = buildXeroxVariantName(specs);
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildXeroxProductDescription(seo.pageIntro, specs),
              warrantyMonths,
              status: CatalogStatus.ACTIVE,
...
void importXeroxProducts().catch((error: unknown) => {
  process.stderr.write(
