...
} from '../src/catalog/razer-product-name';
import {
  buildRazerProductDescription,
  resolveRazerProductSeo,
} from '../src/catalog/razer-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'razer.xlsx');
...
const SUBCATEGORIES_TO_ENSURE: Array<{ slug: string; name: string }> = [
  ...
  { slug: 'gaming-dinamik', name: 'Gaming dinamik' },
  { slug: 'gaming-kreslo', name: 'Gaming kreslo' },
];
...
const BRAND_LOGO_URLS = [
  ...
  'https://cdn.worldvectorlogo.com/logos/razer.svg',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeRazerSku(cellText(raw[4]));
...
function assertSku(model: string): string {
  const sku = normalizeRazerSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `razer-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `razer-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `razer-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('Razer brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'Razer' };
  }
  if (existing.status !== CatalogStatus.ACTIVE || existing.name !== 'Razer') {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'Razer', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'Razer' };
}
...
async function importRazerProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveRazerCatalogName(sku, row.title);
      const seo = resolveRazerProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`razer ${sku}`);
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildRazerProductDescription(seo.pageIntro, specs),
              warrantyMonths,
              status: CatalogStatus.ACTIVE,
...
void importRazerProducts().catch((error: unknown) => {
  process.stderr.write(
