/**
 * One-shot import: Fortinet products from fortinet20.xlsx
 ...
 *
 * Identity is the canonical Fortinet part number.
 * On-request rows are imported with price 0; duplicates by part number are skipped.
 * Photos prefer a non-PDF Excel URL, then the embedded packshot for that row.
 */
...
} from '../src/catalog/fortinet-product-name';
import {
  buildFortinetProductDescription,
  resolveFortinetProductSeo,
} from '../src/catalog/fortinet-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'fortinet20.xlsx');
...
const SUBCATEGORIES_TO_ENSURE: Array<{ slug: string; name: string }> = [
  ...
  { slug: 'ip-telefon', name: 'IP telefon' },
  { slug: 'ip-konfrans-telefonu', name: 'IP Konfrans telefonu' },
  { slug: 'ip-pbx', name: 'IP PBX' },
];
...
const BRAND_LOGO_URLS = [
  'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Fortinet_logo.svg/640px-Fortinet_logo.svg.png',
  'https://www.fortinet.com/content/dam/fortinet-web/logos/fortinet-logo.svg',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeFortinetSku(cellText(raw[4]));
...
function skuModelForGenerator(sku: string): string {
  const compact = sku.replace(/[^A-Za-z0-9]/g, '');
  if (/\d/.test(compact)) {
    return sku;
  }
  return `${sku}-0`;
...
function assertSku(model: string): string {
  const sku = normalizeFortinetSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `fortinet-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `fortinet-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `fortinet-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('Fortinet brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'Fortinet' };
  }
  if (
    existing.status !== CatalogStatus.ACTIVE ||
    existing.name !== 'Fortinet'
  ) {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'Fortinet', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'Fortinet' };
}
...
async function importFortinetProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveFortinetCatalogName(sku, row.title);
      const seo = resolveFortinetProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`fortinet ${sku}`);
      let productSlug = productSlugBase;

      const generatedSku = generateCatalogImportSku({
        brandName: brand.name,
        manufacturerModel: skuModelForGenerator(sku),
        specs: [],
      ...
      if (existingVariant === null) {
        const byModel = await prisma.productVariant.findFirst({
          where: {
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildFortinetProductDescription(
                seo.pageIntro,
                specs,
              ),
              warrantyMonths,
      ...
      await prisma.$transaction(async (tx) => {
        const product = await tx.product.create({
          data: {
            ...
            name: productName,
            slug: productSlug,
            description: buildFortinetProductDescription(seo.pageIntro, specs),
            warrantyMonths,
            status: CatalogStatus.ACTIVE,
            seoTitle: seo.seoTitle,
            seoDescription: seo.seoDescription,
...
void importFortinetProducts().catch((error: unknown) => {
  process.stderr.write(
