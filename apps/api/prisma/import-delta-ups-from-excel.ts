/**
 * One-shot import: Delta UPS products from Delta_UPS_Məhsulları.xlsx
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'Delta_UPS_Məhsulları.xlsx');
...
function imageCandidateUrls(imageUrl: string): string[] {
  const urls = [imageUrl];
  if (imageUrl.endsWith('.jpg.jpg')) {
    urls.push(imageUrl.slice(0, -4));
  }
...
async function downloadImage(
  ...
  for (const candidate of imageCandidateUrls(imageUrl)) {
...
async function importDeltaUpsProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      if (row.brand.toUpperCase() !== 'DELTA') {
      ...
      const productName = resolveDeltaCatalogName(sku, row.title);
      ...
      const seo = resolveDeltaProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
...
void importDeltaUpsProducts().catch((error: unknown) => {
  process.stderr.write(
