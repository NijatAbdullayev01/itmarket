/**
 * One-shot import: EnGenius products from engenius.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the canonical EnGenius model / part number.
 * Duplicate price-list rows (same model, different USD) are skipped.
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'engenius.xlsx');
...
const SUBCATEGORY_SLUG_BY_LABEL: Record<string, string> = {
  ...
  'wireless management kommutator': 'kommutator',
  'access point': 'access-point',
  'wall plate access point': 'access-point',
  'outdoor access point': 'access-point',
...
const BRAND_LOGO_URLS = [
  'https://static.engeniuscdn.com/wp-content/uploads/2023/07/12034239/Group-1096.svg',
  'https://www.engeniustech.com/wp-content/uploads/2023/10/Group-1096.svg',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeEnGeniusSku(cellText(raw[4]));
...
function assertSku(model: string): string {
  const sku = normalizeEnGeniusSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `engenius-catalog-${randomUUID()}`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('EnGenius brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'EnGenius' };
  }
  if (
    existing.status !== CatalogStatus.ACTIVE ||
    existing.name !== 'EnGenius'
  ) {
    await prisma.brand.update({
