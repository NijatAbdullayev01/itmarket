/**
 * One-shot import: Kingston memory from kingston.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the Kingston part number (slash folded to hyphen).
 * Rows without cost or sale price are skipped.
 * Photos prefer the Excel URL (Icecat), then the embedded row image.
 */
...
} from '../src/catalog/kingston-product-name';
import {
  buildKingstonProductDescription,
  resolveKingstonProductSeo,
} from '../src/catalog/kingston-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'kingston.xlsx');
...
const SUBCATEGORIES_TO_ENSURE: Array<{
  ...
  { parentSlug: 'computer', slug: 'ddr5-ram', name: 'DDR5 RAM' },
  { parentSlug: 'computer', slug: 'ssd', name: 'SSD' },
  { parentSlug: 'computer', slug: 'm2-nvme-ssd', name: 'M.2 NVMe SSD' },
  { parentSlug: 'computer', slug: 'usb-flash', name: 'USB flash' },
  { parentSlug: 'computer', slug: 'yaddas-karti', name: 'Yaddaş kartı' },
  { parentSlug: 'computer', slug: 'xarici-ssd', name: 'Xarici SSD' },
  { parentSlug: 'gamer-zona', slug: 'gaming-ram', name: 'Gaming RAM' },
...
const BRAND_LOGO_URLS = [
  'https://cdn.worldvectorlogo.com/logos/kingston-technology.svg',
  'https://www.kingston.com/kingston-assets/media-library/global/logos/kingston-logo.png',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeKingstonSku(partNumber);
...
function assertSku(model: string): string {
  const sku = normalizeKingstonSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `kingston-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `kingston-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `kingston-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('Kingston brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'Kingston' };
  }
  if (
    existing.status !== CatalogStatus.ACTIVE ||
    existing.name !== 'Kingston'
  ) {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'Kingston', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'Kingston' };
}
...
async function importKingstonProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveKingstonCatalogName(sku, row.title);
      const seo = resolveKingstonProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`kingston ${sku}`);
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildKingstonProductDescription(
                seo.pageIntro,
                specs,
              ),
              warrantyMonths,
      ...
      await prisma.$transaction(async (tx) => {
        const product = await tx.product.create({
          data: {
            ...
            name: productName,
            slug: productSlug,
            description: buildKingstonProductDescription(seo.pageIntro, specs),
            warrantyMonths,
            status: CatalogStatus.ACTIVE,
            seoTitle: seo.seoTitle,
            seoDescription: seo.seoDescription,
...
void importKingstonProducts().catch((error: unknown) => {
  process.stderr.write(
