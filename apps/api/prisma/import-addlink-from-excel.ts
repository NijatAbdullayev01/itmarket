/**
 * One-shot import: addlink products from addlink.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the addlink part number (slash folded to hyphen).
 * Rows without cost or sale price are skipped.
 * Photos prefer the Excel URL (addlink.com.tw / Wix), then the embedded row image.
 */
...
} from '../src/catalog/addlink-product-name';
import {
  buildAddlinkProductDescription,
  resolveAddlinkProductSeo,
} from '../src/catalog/addlink-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'addlink.xlsx');
...
const BRAND_LOGO_URLS = [
  'https://static.wixstatic.com/media/ef4c6b_96145ab64e11426c9ecfcbdfcf269ab3~mv2.png',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeAddlinkSku(partNumber);
...
function assertSku(model: string): string {
  const sku = normalizeAddlinkSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `addlink-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `addlink-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `addlink-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('addlink brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'addlink' };
  }
  if (
    existing.status !== CatalogStatus.ACTIVE ||
    existing.name !== 'addlink'
  ) {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'addlink', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'addlink' };
}
...
async function importAddlinkProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveAddlinkCatalogName(sku, row.title);
      const seo = resolveAddlinkProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`addlink ${sku}`);
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildAddlinkProductDescription(
                seo.pageIntro,
                specs,
              ),
              warrantyMonths,
      ...
      await prisma.$transaction(async (tx) => {
        const product = await tx.product.create({
          data: {
            ...
            name: productName,
            slug: productSlug,
            description: buildAddlinkProductDescription(seo.pageIntro, specs),
            warrantyMonths,
            status: CatalogStatus.ACTIVE,
            seoTitle: seo.seoTitle,
            seoDescription: seo.seoDescription,
...
void importAddlinkProducts().catch((error: unknown) => {
  process.stderr.write(
