/**
 * One-shot import: Grandstream products from Grandstream_Məhsulları.xlsx
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'Grandstream_Məhsulları.xlsx');
...
const SUBCATEGORIES_TO_ENSURE: Array<{ slug: string; name: string }> = [
  ...
  { slug: 'ip-video-telefon', name: 'IP Video telefon' },
  { slug: 'ip-dect-telefon', name: 'IP DECT telefon' },
  { slug: 'ip-wi-fi-telefon', name: 'IP Wi-Fi telefon' },
  { slug: 'ip-konfrans-telefonu', name: 'IP Konfrans telefonu' },
  { slug: 'ip-pbx', name: 'IP PBX' },
  { slug: 'ip-telefon-aksesuarlari', name: 'IP telefon aksesuarları' },
...
const SUBCATEGORY_SLUG_BY_LABEL: Record<string, string> = {
  ...
  'ip wifi telefon': 'ip-wi-fi-telefon',
  'ip konfrans telefonu': 'ip-konfrans-telefonu',
  'ip pbx': 'ip-pbx',
  'ip telefon aksesuarlari': 'ip-telefon-aksesuarlari',
  'ip telefon aksesuarları': 'ip-telefon-aksesuarlari',
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `grandstream-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `grandstream-out-${randomUUID()}.jpg`);
...
async function fetchImageBody(imageUrl: string): Promise<Buffer | null> {
  const response = await fetch(imageUrl, {
    ...
    headers: {
      ...
      Referer: 'https://www.grandstream.com/',
...
function assertSku(model: string): string {
  const sku = normalizeGrandstreamSku(model);
...
async function ensureBrand(
  ...
  const existing = await prisma.brand.findUnique({
    where: { slug: 'grandstream' },
    select: { id: true, name: true, status: true },
  });
  if (existing !== null) {
    if (
      existing.status !== CatalogStatus.ACTIVE ||
      existing.name !== 'Grandstream'
    ) {
      await prisma.brand.update({
        where: { id: existing.id },
        data: { name: 'Grandstream', status: CatalogStatus.ACTIVE },
      });
    }
    return { id: existing.id, name: 'Grandstream' };
  }
...
async function importGrandstreamProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      if (row.brand.toUpperCase() !== 'GRANDSTREAM') {
      ...
      const inferredSlug = inferGrandstreamSubcategorySlug(sku);
      const subcategorySlug =
        inferredSlug === 'sfp-modullar'
          ? inferredSlug
          : resolveSubcategorySlug(row.subCategory);
...
void importGrandstreamProducts().catch((error: unknown) => {
  process.stderr.write(
