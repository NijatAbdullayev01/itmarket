/**
 * One-shot import: HyperX by HP products from hyperx.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the HyperX / HP part number. Each P/N is a separate product.
...
} from '../src/catalog/hyperx-product-name';
import {
  buildHyperxProductDescription,
  resolveHyperxProductSeo,
} from '../src/catalog/hyperx-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'hyperx.xlsx');

const PARENT_SLUG = 'gamer-zona';
const PARENT_NAME = 'Gamer zona';

const SUBCATEGORIES_TO_ENSURE: Array<{ slug: string; name: string }> = [
  { slug: 'gaming-monitor', name: 'Gaming monitor' },
  { slug: 'gaming-klaviatura', name: 'Gaming klaviatura' },
  { slug: 'gaming-sican', name: 'Gaming siçan' },
  { slug: 'gaming-qulaqliq', name: 'Gaming qulaqlıq' },
  { slug: 'gaming-canta', name: 'Gaming çanta' },
  { slug: 'gaming-mikrofon', name: 'Gaming mikrofon' },
  { slug: 'gaming-sican-altligi', name: 'Gaming siçan altlığı' },
  { slug: 'gaming-veb-kamera', name: 'Gaming veb kamera' },
  { slug: 'gaming-pult', name: 'Gaming pult' },
  { slug: 'gaming-sarj-stansiyasi', name: 'Gaming şarj stansiyası' },
  { slug: 'gaming-audio-mikser', name: 'Gaming audio mikser' },
];
...
const SUBCATEGORY_SLUG_BY_LABEL: Record<string, string> = {
  ...
  'gaming sarj stansiyasi': 'gaming-sarj-stansiyasi',
...
const BRAND_LOGO_URLS = [
  'https://upload.wikimedia.org/wikipedia/commons/0/0f/HyperX_logo-for-DIGITAL-use.png',
  'https://upload.wikimedia.org/wikipedia/commons/thumb/0/0f/HyperX_logo-for-DIGITAL-use.png/640px-HyperX_logo-for-DIGITAL-use.png',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeHyperxSku(cellText(raw[4]));
...
function assertSku(model: string): string {
  const sku = normalizeHyperxSku(model);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `hyperx-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `hyperx-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `hyperx-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('HyperX brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'HyperX' };
  }
  if (existing.status !== CatalogStatus.ACTIVE || existing.name !== 'HyperX') {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'HyperX', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'HyperX' };
}
...
async function ensureSubcategories(
  ...
  if (parent.parentId !== null) {
    throw new Error('Expected root category: gamer-zona');
  }
...
async function importHyperxProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      if (normalizeCategoryKey(row.mainCategory) !== 'gamer zona') {
        throw new Error(
      ...
      const productName = resolveHyperxCatalogName(sku, row.title, {
        subcategorySlug,
        specs,
      });
      const seo = resolveHyperxProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`hyperx ${sku}`);
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildHyperxProductDescription(seo.pageIntro, specs),
              warrantyMonths,
              status: CatalogStatus.ACTIVE,
...
void importHyperxProducts().catch((error: unknown) => {
  process.stderr.write(
