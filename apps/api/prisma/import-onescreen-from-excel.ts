/**
 * One-shot import: OneScreen products from onescreen.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the supplier part number. Same marketing panel may appear
 * under several SKUs; each P/N is a separate product so prices stay distinct.
 * Photos prefer the Excel URL, then the embedded row image.
 */
...
import { findExistingImportedVariant } from '../src/catalog/catalog-import-identity';
import { normalizeVariantBarcode } from '../src/catalog/variant.domain';
import {
  normalizeOneScreenSku,
  resolveOneScreenCatalogName,
} from '../src/catalog/onescreen-product-name';
import {
  buildOneScreenProductDescription,
  resolveOneScreenProductSeo,
} from '../src/catalog/onescreen-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'onescreen.xlsx');
...
const SUB_SLUG_BY_PARENT_AND_LABEL: Record<string, Record<string, string>> = {
  monitorlar: {
    'interaktiv lovhe': 'interaktiv-lovhe',
  ...
  'sebeke avadanliqlari': {
    'ekran paylasimi': 'ekran-paylasimi',
  },
  'kamera ve foto': {
    'veb kamera': 'konfrans-kamerasi',
...
const PARENTS_TO_ENSURE: Array<{ slug: string; name: string }> = [
  ...
  { slug: 'kamera-foto', name: 'Kamera və foto' },
];
...
const SUBCATEGORIES_TO_ENSURE: Array<{
...
}> = [
  {
    ...
    slug: 'interaktiv-lovhe',
    name: 'Interaktiv lövhə',
  },
  {
    parentSlug: 'monitorlar',
    slug: 'monitor-stendi',
  ...
  { parentSlug: 'computer', slug: 'mini-pc', name: 'Mini PC' },
  {
    parentSlug: 'sebeke-avadanliqlari',
    slug: 'ekran-paylasimi',
...
const BRAND_LOGO_URLS = [
  'https://www.onescreensolutions.com/wp-content/uploads/2025/07/Core-1.webp',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeOneScreenSku(partNumber);
...
function assertSku(model: string): string {
  const sku = normalizeOneScreenSku(model);
...
/** Keep full manufacturer codes — Cast/WebCam have no digits and would collide.
 *  Prefix is ONESCR- (not OS-) so part numbers like OS-T7-75 are not mistaken
 *  for the generated variant SKU of T7-75. */
function buildOneScreenVariantSku(sku: string): string {
  const token = sku.replace(/[^A-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '');
  const candidate = `ONESCR-${token}`.slice(0, 64);
  if (!/^[A-Z0-9][A-Z0-9._-]{1,63}$/.test(candidate)) {
    throw new Error(`Invalid generated SKU for ${sku}`);
  }
  return candidate;
}
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `onescreen-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `onescreen-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `onescreen-out-${randomUUID()}.jpg`);
...
function referersFor(imageUrl: string): Array<string | undefined> {
  const referers: Array<string | undefined> = [
    'https://www.onescreensolutions.com/',
    'https://partnerportal.onescreensolutions.com/',
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('OneScreen brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'OneScreen' };
  }
  if (
    existing.status !== CatalogStatus.ACTIVE ||
    existing.name !== 'OneScreen'
  ) {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'OneScreen', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'OneScreen' };
}
...
async function importOneScreenProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveOneScreenCatalogName(sku, row.title);
      const seo = resolveOneScreenProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`onescreen ${sku}`);
      let productSlug = productSlugBase;

      const generatedSku = buildOneScreenVariantSku(sku);
      ...
      // Match by generated SKU / legacy part number only — do not match by
      // marketing name, because several supplier SKUs share the same panel title.
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildOneScreenProductDescription(
                seo.pageIntro,
                specs,
              ),
              warrantyMonths,
      ...
      await prisma.$transaction(async (tx) => {
        const product = await tx.product.create({
          data: {
            ...
            name: productName,
            slug: productSlug,
            description: buildOneScreenProductDescription(seo.pageIntro, specs),
            warrantyMonths,
            status: CatalogStatus.ACTIVE,
            seoTitle: seo.seoTitle,
            seoDescription: seo.seoDescription,
...
void importOneScreenProducts().catch((error: unknown) => {
  process.stderr.write(
