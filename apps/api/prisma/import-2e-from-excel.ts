...
import {
  ...
  twoeSkuForGenerator,
} from '../src/catalog/twoe-product-name';
import {
  buildTwoEProductDescription,
  resolveTwoEProductSeo,
} from '../src/catalog/twoe-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, '2e.xlsx');
...
const SUB_SLUG_BY_PARENT_AND_LABEL: Record<string, Record<string, string>> = {
  'gamer zona': {
    ...
    'gaming qulaqliq': 'gaming-qulaqliq',
    'gaming sican': 'gaming-sican',
    'gaming sican altligi': 'gaming-sican-altligi',
  },
  'komputer ve komponentleri': {
    'elektrik uzadici': 'elektrik-uzadici',
    klaviatura: 'klaviatura',
    'klaviatura ve sican desti': 'klaviatura-ve-sican-desti',
    korpus: 'korpus',
    sican: 'sican',
    'temizlik desti': 'temizlik-desti',
    'usb hub': 'usb-hub',
  },
  monitorlar: {
    monitor: 'monitor',
  ...
  printerler: {
    'kagiz dograyan': 'kagiz-dograyan',
  },
  'tv ve audio': {
    mikrofon: 'mikrofon',
    'portativ kolonka': 'portativ-kolonka',
    qulaqliq: 'qulaqliq',
    'tv tutacagi': 'tv-tutacagi',
    televizor: 'televizor',
  },
  ups: {
...
const SUBCATEGORIES_TO_ENSURE: Array<{
  ...
  { parentSlug: 'gamer-zona', slug: 'gpu-dayaq', name: 'GPU dayaq' },
  { parentSlug: 'gamer-zona', slug: 'gaming-dinamik', name: 'Gaming dinamik' },
  { parentSlug: 'gamer-zona', slug: 'gaming-eynek', name: 'Gaming eynək' },
  {
    parentSlug: 'gamer-zona',
    slug: 'gaming-klaviatura',
    name: 'Gaming klaviatura',
  },
  { parentSlug: 'gamer-zona', slug: 'gaming-kreslo', name: 'Gaming kreslo' },
  { parentSlug: 'gamer-zona', slug: 'gaming-lampa', name: 'Gaming lampa' },
  { parentSlug: 'gamer-zona', slug: 'gaming-masa', name: 'Gaming masa' },
  { parentSlug: 'gamer-zona', slug: 'gaming-mikrofon', name: 'Gaming mikrofon' },
  { parentSlug: 'gamer-zona', slug: 'gaming-monitor', name: 'Gaming monitor' },
  { parentSlug: 'gamer-zona', slug: 'gaming-qulaqliq', name: 'Gaming qulaqlıq' },
  { parentSlug: 'gamer-zona', slug: 'gaming-sican', name: 'Gaming siçan' },
  {
    parentSlug: 'gamer-zona',
    slug: 'gaming-sican-altligi',
    name: 'Gaming siçan altlığı',
  },
  {
    parentSlug: 'computer',
    slug: 'elektrik-uzadici',
    name: 'Elektrik uzadıcı',
  },
  ...
  { parentSlug: 'computer', slug: 'korpus', name: 'Korpus' },
  { parentSlug: 'computer', slug: 'sican', name: 'Siçan' },
  { parentSlug: 'computer', slug: 'temizlik-desti', name: 'Təmizlik dəsti' },
  { parentSlug: 'computer', slug: 'usb-hub', name: 'USB hub' },
  { parentSlug: 'monitorlar', slug: 'monitor', name: 'Monitor' },
  { parentSlug: 'monitorlar', slug: 'monitor-stendi', name: 'Monitor stendi' },
  {
  ...
  { parentSlug: 'tv-audio', slug: 'tv-tutacagi', name: 'TV tutacağı' },
  { parentSlug: 'tv-audio', slug: 'televizor', name: 'Televizor' },
  {
    parentSlug: 'ups',
    slug: 'line-interactive',
...
const BRAND_LOGO_URLS = [
  'https://2e-world.com/wp-content/uploads/2023/06/2E-logo.svg',
  'https://2egaming.com/wp-content/uploads/2021/05/2e-gaming-logo.svg',
  'https://2e-world.com/wp-content/uploads/2021/03/cropped-2E-favicon-192x192.png',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeTwoESku(partNumber);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `twoe-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `twoe-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `twoe-out-${randomUUID()}.jpg`);
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('2e brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: '2E' };
  }
  if (existing.status !== CatalogStatus.ACTIVE || existing.name !== '2E') {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: '2E', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: '2E' };
}
...
async function importTwoEProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveTwoECatalogName(sku, row.title);
      ...
      const productSlugBase = slugifyCatalogLabel(`2e ${sku}`);
      let productSlug = productSlugBase;

      const generatedSku = generateCatalogImportSku({
        brandName: brand.name,
        manufacturerModel: twoeSkuForGenerator(sku),
        specs: [],
      ...
      let existingVariant = await prisma.productVariant.findFirst({
        where: {
          ...
          attributes: { path: ['Model'], equals: sku },
        },
        select: { id: true, productId: true },
      });
      if (existingVariant === null) {
        existingVariant = await findExistingImportedVariant(prisma, {
          brandId: brand.id,
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildTwoEProductDescription(seo.pageIntro, specs),
              warrantyMonths,
              status: CatalogStatus.ACTIVE,
...
void importTwoEProducts().catch((error: unknown) => {
  process.stderr.write(
