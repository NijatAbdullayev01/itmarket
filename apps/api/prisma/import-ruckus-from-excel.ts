/**
 * One-shot import: Ruckus products from Ruckus_Məhsulları.xlsx
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'Ruckus_Məhsulları.xlsx');
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `ruckus-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `ruckus-out-${randomUUID()}.jpg`);
...
function referersFor(imageUrl: string): Array<string | undefined> {
  const referers: Array<string | undefined> = [
    ...
    'https://productresources.vistancenetworks.com/',
  ];
  try {
...
function assertSku(model: string): string {
  const sku = normalizeRuckusSku(model);
...
async function ensureBrand(
  ...
  if (existing !== null) {
    if (
      ...
      existing.name !== 'Ruckus'
    ) {
      await prisma.brand.update({
        where: { id: existing.id },
        data: { name: 'Ruckus', status: CatalogStatus.ACTIVE },
      });
    }
    return { id: existing.id, name: 'Ruckus' };
  }
...
async function importRuckusProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      if (row.brand.toUpperCase() !== 'RUCKUS') {
      ...
      const productName = resolveRuckusCatalogName(sku, row.title, {
        subcategorySlug,
        specs,
      });
      const seo = resolveRuckusProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`ruckus ${sku}`);
...
void importRuckusProducts().catch((error: unknown) => {
  process.stderr.write(
