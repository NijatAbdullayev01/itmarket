...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `qnap-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `qnap-out-${randomUUID()}.jpg`);
...
async function importQnapProducts(): Promise<void> {
...
void importQnapProducts().catch((error: unknown) => {
  process.stderr.write(
