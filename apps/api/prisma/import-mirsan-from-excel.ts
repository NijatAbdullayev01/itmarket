/**
 * One-shot import: Mirsan products from mirsan.xlsx
 * Maya AZN → variant.cost; Satış qiyməti AZN (+25%) → variant.price
 * Variants are created with availableByOrder=true (sifarişlə).
 *
 * Identity is the Mirsan part number (plus folded to hyphen).
 * Rows without cost or sale price are skipped.
 * Photos prefer the Excel URL (mirsanrack.com), then the embedded row image.
 */
...
} from '../src/catalog/mirsan-product-name';
import {
  buildMirsanProductDescription,
  resolveMirsanProductSeo,
} from '../src/catalog/mirsan-product-seo';
...
const EXCEL_PATH = path.join(WORKSPACE_ROOT, 'mirsan.xlsx');
...
const SUB_SLUG_BY_PARENT_AND_LABEL: Record<string, Record<string, string>> = {
  server: {
    pdu: 'pdu',
    'rack aksesuarlari': 'rack-aksesuarlari',
    'server skafi': 'server-skafi',
    'sebeke skafi': 'sebeke-skafi',
  },
  'sebeke avadanliqlari': {
    'divar skafi': 'divar-skafi',
  },
};
...
const SUBCATEGORIES_TO_ENSURE: Array<{
  ...
  { parentSlug: 'server', slug: 'pdu', name: 'PDU' },
  {
    parentSlug: 'server',
    slug: 'rack-aksesuarlari',
    name: 'Rack aksesuarları',
  },
  { parentSlug: 'server', slug: 'server-skafi', name: 'Server şkafı' },
  { parentSlug: 'server', slug: 'sebeke-skafi', name: 'Şəbəkə şkafı' },
  {
...
const BRAND_LOGO_URLS = [
  'https://www.mirsanrack.com/wp-content/uploads/2020/12/logo-mirsan-orjinal-3-01.png',
];
...
function readExcelRows(): ExcelRow[] {
  ...
  for (const [index, raw] of matrix.entries()) {
    ...
    const sku = normalizeMirsanSku(partNumber);
...
async function unzipCatalogEntries(): Promise<Map<string, Buffer>> {
  const dest = path.join(tmpdir(), `mirsan-catalog-${randomUUID()}`);
...
async function compressCatalogImage(body: Buffer): Promise<{
  ...
  const inputPath = path.join(tmpdir(), `mirsan-in-${randomUUID()}`);
  const outputPath = path.join(tmpdir(), `mirsan-out-${randomUUID()}.jpg`);
...
function referersFor(imageUrl: string): Array<string | undefined> {
  const referers: Array<string | undefined> = [
    'https://www.mirsanrack.com/',
    'https://mirsanrack.com/',
  ];
  try {
...
async function attachBrandLogoIfMissing(
  ...
  process.stderr.write('mirsan brand logo download failed\n');
}
...
async function ensureBrand(
  ...
  if (existing === null) {
    ...
    return { id: created.id, name: 'Mirsan' };
  }
  if (existing.status !== CatalogStatus.ACTIVE || existing.name !== 'Mirsan') {
    await prisma.brand.update({
      where: { id: existing.id },
      data: { name: 'Mirsan', status: CatalogStatus.ACTIVE },
    });
  }
  await attachBrandLogoIfMissing(prisma, existing.id, existing.logoObjectKey);
  return { id: existing.id, name: 'Mirsan' };
}
...
async function importMirsanProducts(): Promise<void> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const productName = resolveMirsanCatalogName(sku, row.title);
      const seo = resolveMirsanProductSeo({
        sku,
        title: productName,
        specs,
        subcategorySlug,
      ...
      const productSlugBase = slugifyCatalogLabel(`mirsan ${sku}`);
      let productSlug = productSlugBase;

      const generatedSku = generateCatalogImportSku({
        brandName: brand.name,
        manufacturerModel: mirsanSkuForGenerator(sku),
        specs: [],
        ...
        await prisma.$transaction(async (tx) => {
          await tx.product.update({
            ...
            data: {
              ...
              brandId: brand.id,
              name: productName,
              description: buildMirsanProductDescription(seo.pageIntro, specs),
              warrantyMonths,
              status: CatalogStatus.ACTIVE,
...
void importMirsanProducts().catch((error: unknown) => {
  process.stderr.write(
