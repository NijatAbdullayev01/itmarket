...
describe('Phase 7 security and production-readiness integration', () => {
  ...
  it('rejects staff session cookies on customer-only endpoints', async () => {
    const agent = request.agent(app.getHttpServer());
    const email = `staff-boundary-${suffix}-${randomUUID().slice(0, 4)}@example.invalid`;
    const password = 'phase7-staff-boundary-password';
    ...
    await agent.get('/api/v1/customer/auth/me').expect(401);
    await agent.get('/api/v1/customer/me').expect(401);
    await agent.get('/api/v1/customer/orders').expect(401);
  });

  it('rejects a stale signed webhook outside the replay window', async () => {
    const fixture = await createSellableFixture(1);
    const checkout = await createOnlineCheckout(
      fixture.variantId,
      fixture.deliveryZoneId,
