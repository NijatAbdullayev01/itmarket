...
import { listTranscendCatalogNameSkus } from './transcend-product-name';
import {
  listHandcraftedTranscendSkus,
  resolveTranscendProductSeo,
} from './transcend-product-seo';

const PRICE_PROMISE =
  /(ən\s+ucuz|ən\s+sərfəli\s+qiymət|ucuz\s+qiymət|endirim\s+zəmanəti|0\s*azn)/iu;

describe('transcend-product-seo', () => {
  const skus = listHandcraftedTranscendSkus();
  const copies = skus.map((sku) => ({
    ...
    ...resolveTranscendProductSeo({
      ...
      title: `Transcend ${sku}`,
      specs: [],
      subcategorySlug: sku.includes('SJ25') ? 'xarici-hdd' : 'xarici-ssd',
    }),
  }));

  it('covers every Transcend Excel SKU', () => {
    expect(skus).toEqual(listTranscendCatalogNameSkus());
  ...
  it('writes unique Azerbaijani copy without site suffix or price promises', () => {
    ...
    for (const copy of copies) {
      expect(copy.seoTitle).toMatch(/Transcend/i);
