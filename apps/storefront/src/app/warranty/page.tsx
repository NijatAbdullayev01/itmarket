...
import { IconWarranty } from "@itmarket/ui";
import { getRequestLocale } from "@/lib/i18n/get-locale";
import { DEFAULT_LOCALE } from "@/lib/i18n";
import {
  WARRANTY_CONTACT_EMAIL,
  WARRANTY_CONTACT_PHONES,
...
export async function generateMetadata(): Promise<Metadata> {
  const content = getWarrantyPageContent(DEFAULT_LOCALE);
      ...
      <div className="ui-legal-page__card">
        <header className="ui-legal-page__header">
          <div className="ui-legal-page__header-icon" aria-hidden="true">
            <IconWarranty width={28} height={28} />
          </div>
          ...
          <section>
            ...
            <ul>
              <li>
                ...
                <a href={`mailto:${WARRANTY_CONTACT_EMAIL}`}>
                  {WARRANTY_CONTACT_EMAIL}
                </a>
              </li>
              <li>
                {content.contact.phoneLabel}:{" "}
                {WARRANTY_CONTACT_PHONES.map((phone, index) => (
                  <span key={phone.href}>
                    {index > 0 ? " · " : null}
