...
import { IconStore } from "@itmarket/ui";
import { getRequestLocale } from "@/lib/i18n/get-locale";
import { DEFAULT_LOCALE } from "@/lib/i18n";
import {
  ABOUT_CONTACT_EMAIL,
  ABOUT_CONTACT_PHONES,
  getAboutPageContent,
  type AboutBlock,
...
export async function generateMetadata(): Promise<Metadata> {
  const content = getAboutPageContent(DEFAULT_LOCALE);
        ...
        <article className="ui-legal-content">
          ...
          {bodySections.map((section) => (
            <section key={section.title}>
              ...
              {section.blocks.map((block, index) => (
                <AboutBlockView key={`${section.title}-${index}`} block={block} />
              ))}
            </section>
          ))}
          ...
          <section>
            ...
            {contactSection.blocks.map((block, index) => (
              <AboutBlockView key={`contact-intro-${index}`} block={block} />
            ))}
            <ul>
              <li>
                {content.contact.emailLabel}:{" "}
                <a href={`mailto:${ABOUT_CONTACT_EMAIL}`}>
                  {ABOUT_CONTACT_EMAIL}
                </a>
              </li>
              <li>
                {content.contact.phoneLabel}:{" "}
                {ABOUT_CONTACT_PHONES.map((phone, index) => (
                  <span key={phone.href}>
                    {index > 0 ? " · " : null}
