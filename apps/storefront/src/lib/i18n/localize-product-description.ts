...
const INTRO_NAME_CAPTURE = "([\\p{L}\\p{N}][\\p{L}\\p{N} .\\-\\/&]{0,60}?)";

const BRAND_INTRO_TEMPLATE =
...
const SUBCATEGORY_INTRO_TEMPLATE =
...
function buildIntroTemplateRe(template: string): RegExp {
  const sentinel = "\uE000";
  const marked = template.replace(/\{(name|parent)\}/g, sentinel);
  const escaped = escapeRegExp(marked).replace(
...
const BRAND_INTRO_RE = buildIntroTemplateRe(BRAND_INTRO_TEMPLATE);
const CATEGORY_INTRO_RE = buildIntroTemplateRe(CATEGORY_INTRO_TEMPLATE);
const SUBCATEGORY_INTRO_RE = buildIntroTemplateRe(SUBCATEGORY_INTRO_TEMPLATE);

function resolveIntroEntityName(
  azName: string,
  locale: Exclude<Locale, "az">,
  localizedName?: string,
): string {
  const override = localizedName?.trim();
  return override ? override : localizeProseRemainder(azName.trim(), locale);
}

/**
 * Localize the fixed brand / category landing descriptions generated by the
 * SEO heuristic into EN/RU. The entity name is captured and substituted with
 * the caller-provided localized name (or fragment-translated as a fallback).
 */
function applyCatalogIntroTemplates(
  text: string,
  locale: Exclude<Locale, "az">,
  ...
  result = result.replace(
    ...
    (_match, name: string) =>
      locale === "en"
        ? `Discover the official ${name.trim()} products in the IT Market storefront. Compare smartphone, laptop, and other tech models in the catalog and choose the right configuration. Shop with original products, official warranty, professional support, and convenient delivery / in-store pickup options.`
        : `Откройте официальные товары бренда ${name.trim()} в витрине IT Market. Сравните модели смартфонов, ноутбуков и другой техники в каталоге и выберите подходящую конфигурацию. Покупайте с оригинальными товарами, официальной гарантией, профессиональной поддержкой и удобной доставкой / самовывозом из магазина.`,
  );

  result = result.replace(
    CATEGORY_INTRO_RE,
    (_match, name: string) => {
      const localized = resolveIntroEntityName(name, locale, localizedEntityName);
      return locale === "en"
        ? `Find products in the ${localized} category at IT Market. Compare brands and models and choose the configuration that suits your needs. Original equipment, official warranty, professional support, and convenient delivery / in-store pickup options are provided.`
        : `Найдите товары в категории «${localized}» в IT Market. Сравните бренды и модели и выберите конфигурацию под ваши задачи. Предлагаются оригинальная техника, официальная гарантия, профессиональная поддержка и удобная доставка / самовывоз из магазина.`;
    },
  );

  result = result.replace(
    SUBCATEGORY_INTRO_RE,
    (_match, parent: string, name: string) => {
      const localizedParent = localizeProseRemainder(parent.trim(), locale);
      const localizedName = localizeProseRemainder(name.trim(), locale);
      return locale === "en"
        ? `${localizedName} options in the ${localizedParent} category — compare by brand, model, and key features at IT Market. Shop with original equipment, official warranty, professional support, and convenient delivery / in-store pickup. Quickly find the right product in the catalog using filters and sorting.`
        : `Варианты ${localizedName} в категории ${localizedParent} — сравните по бренду, модели и ключевым характеристикам в IT Market. Покупайте с оригинальной техникой, официальной гарантией, профессиональной поддержкой и удобной доставкой / самовывозом из магазина. Быстро найдите нужный товар в каталоге с помощью фильтров и сортировки.`;
    },
  );
...
/**
 * Localize stored Azerbaijani brand / category landing descriptions for the
 * storefront locale. Reuses the same prose pipeline as product descriptions,
 * plus fixed template translations for the generated brand/category copy.
 *
 * When a category `slug` is provided, a curated exact translation (if present
 * in `CATALOG_INTRO_TRANSLATIONS`) takes precedence — used for long, hand-
 * written category intros that the generic pipeline cannot translate well.
 */
export function localizeCatalogIntro(
  description: string | null | undefined,
  locale: Locale,
  localizedEntityName?: string,
  slug?: string | null,
  ...
  const override =
    typeof slug === "string" ? CATALOG_INTRO_TRANSLATIONS[slug] : undefined;
  ...
  return source
    ...
    .map((line) => {
      ...
      let result = applyCatalogIntroTemplates(line, locale, localizedEntityName);
      result = applyBrandTemplates(result, locale);
      result = applyPhraseMap(result, locale);
      result = localizeProseRemainder(result, locale);
      return result.replace(/\s{2,}/g, " ").trim();
    })
