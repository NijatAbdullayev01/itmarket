...
| --- | --- | --- | --- |
| 0 | Discovery və qərarlar | **Tamamlanıb** | [phase-0-discovery-summary.md](phase-0-discovery-summary.md) |
| 1 | Foundation | **Tamamlanıb** | [roadmap.md](roadmap.md) — frozen install, CI, migration, build |
| 2 | Auth, catalog və inventory | **Tamamlanıb** | [modules/auth-catalog-inventory.md](modules/auth-catalog-inventory.md) |
| 3 | Storefront, cart və checkout | **Tamamlanıb** | [modules/storefront-cart-checkout.md](modules/storefront-cart-checkout.md) |
| 4 | Online payment və fulfillment | **Kod tamamlanıb; xarici gate açıq** | [modules/online-payment-fulfillment.md](modules/online-payment-fulfillment.md); mock + Epoint adapter, callback, fulfillment, refund/cancel; PostgreSQL acceptance doğrulanıb; merchant credential və canlı sandbox rehearsal gate-ləri açıq |
| 5 | POS və kassa növbəsi | **Kod tamamlanıb; fiscal gate açıq** | [modules/pos-cash-register.md](modules/pos-cash-register.md); installment metadata, thermal receipt, PostgreSQL acceptance doğrulanıb |
| 6 | Hesabatlar | **Başlanıb; acceptance doğrulanıb** | [modules/reports.md](modules/reports.md); PostgreSQL acceptance suite keçir |
| 7 | Hardening və production readiness | **Engineering tamamlanıb; production acceptance NO-GO** | [phase-7-production-readiness.md](phase-7-production-readiness.md) |

**Vacib — faza bağlanış qaydası:** Hər faza qəbul meyarları bağlanandan sonra agent **eyni iş sessiyasının sonunda** aşağıdakıları yeniləməlidir:

1. bu sənəddəki **Cari repo faza vəziyyəti** cədvəli (tarix, tamamlanmış faza sayı, cari prioritet, hər faza üçün status);
2. `docs/roadmap.md`-də həmin fazanın **Vəziyyət** sətri;
3. əlaqəli modul sənədi (`docs/modules/...`) — nə tamamlandı, hansı acceptance test-ləri keçdi, nə açıq qaldı;
4. lazım olduqda §13-də faza başlığının status badge-i.

Tamamlanmış faza üçün qeyd ediləcək minimum məlumat: faza nömrəsi/adı, status (`Tamamlanıb` / `İcra olunur` / `Başlanıb`), sübut (keçən test/build komandaları, modul sənəd linki), açıq qalan gate-lər.

Faza tamamlanmadan növbəti fazanı “tamamlandı” kimi qeyd etmə. Bu qeydlər növbəti agent üçün source of truth-dur — yeni sessiya bütün repo kodunu sıfırdan audit etməməlidir.
...
3. Repo boş deyilsə, əvvəl `docs/itmarket-cursor-master-prompt.md`-dəki **Cari repo faza vəziyyəti** cədvəli və `docs/roadmap.md`-i oxu. **Tamamlanmış** (`Tamamlanıb`) fazaları yenidən kod auditindən keçirmə; onları sənədləşdirilmiş sübut kimi qəbul et və yalnız cari/açıq faza, sənəd-kod ziddiyyəti və ya istifadəçinin konkret tapşırığı üçün lazım olan modullara bax. Sonra hansı acceptance criterion-un tamamlandığını və hansının açıq qaldığını qısa yaz.
...
6. Bütün layihəni bir böyük dəyişiklik kimi yaratma. Aşağıdakı fazaları ardıcıllıqla icra et və hər fazanın sonunda:
   ...
   - **faza bağlanış qeydini yenilə:** `Cari repo faza vəziyyəti` cədvəli, `docs/roadmap.md`, əlaqəli modul sənədi və lazım gələrsə §13 status badge-i.
...
17. Faza tamamlananda və ya statusu dəyişəndə faza bağlanış qeydini buraxma. Növbəti agentin bütün kod bazasını yenidən yoxlamaması üçün tamamlanan işi sənədlərdə açıq qeyd et; yalnız cari faza və açıq gap-lər üçün kod oxumağa get.
...
- ilk addım kimi **Cari repo faza vəziyyəti** cədvəli və `docs/roadmap.md`-dən cari faza xəritəsini oxu; tamamlanmış fazaları sübut kimi qəbul et;
- tamamlanmış fazalar üçün tam kod auditini aparma; yalnız cari/açıq faza, dəyişəcəyin modul və sənəd-kod ziddiyyəti şübhəsi olduqda kod oxu;
...
Cari tamamlanma vəziyyəti yuxarıdakı **Cari repo faza vəziyyəti** cədvəlində saxlanılır. Faza bağlananda həm həmin cədvəli, həm `docs/roadmap.md`-i, həm də əlaqəli modul sənədini eyni sessiyada yenilə. Növbəti agent tamamlanmış fazaları bu qeydlərdən oxuyur, bütün kod bazasını yenidən audit etmir.

#### Faza 0 — Discovery və qərarlar ✅ Tamamlanıb
...
#### Faza 2 — Auth, catalog və inventory ✅ Tamamlanıb
...
#### Faza 3 — Storefront, cart və checkout ✅ Tamamlanıb
...
#### Faza 4 — Online payment və fulfillment 🔄 İcra olunur
...
#### Faza 5 — POS və kassa növbəsi ✅ Kod tamamlanıb; fiscal gate açıq
...
#### Faza 7 — Hardening və production readiness ⚠️ Engineering tamamlanıb; production acceptance NO-GO
...
1. workspace üzrə qısa tapıntı yaz (struktur, aktiv branch, açıq fayllar); bütün repo-nu faza-faza kod auditindən keçirmə.
2. **Cari repo faza vəziyyəti** cədvəli və `docs/roadmap.md` əsasında tamamlanmış vs açıq fazaları göstər; yalnız cari prioritet faza və açıq acceptance gap-ləri detallı yaz.
3. qəbul etdiyin fərziyyələri və yalnız həqiqətən bloklayan sualları qeyd et;
4. fazalara bölünmüş konkret icra planı ver — əsasən **ən yüksək prioritetli açıq faza** üçün;
5. yaradacağın və ya dəyişəcəyin əsas fayl, modul və sənədləri sadala;
6. cari faza üçün verification komandalarını göstər;
...
9. faza tamamlayanda faza bağlanış qeydini yeniləməyi unutma (cədvəl, roadmap, modul sənədi).
